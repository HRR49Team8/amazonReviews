...
const insertUsers = async () => {
  for (let i = 0; i < 100; i++) {
    ...
    const randomAvatar = s3AvatarDomain + randomAvatars[i];
    const params = [randomUserName, randomCountry, randomAvatar];
    const queryStr = 'INSERT INTO users(user_name, country, avatar) VALUES (?, ?, ?)';
...
const insertReviews = async () => {
  ...
  for (let i = 1; i <= 100; i++) {
    ...
    for (let j = 0; j < numberOfReviews; j++) {
      ...
      const review_date = faker.date.past(5, '2020-11-13');
      ...
      const queryStr = 'INSERT INTO reviews(product_id, user_id, overall_rating, review_date, headline, full_text, helpful, verified_purchase, product_photo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
